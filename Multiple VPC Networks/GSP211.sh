...
echo "${BG_BLUE}${WHITE}${BOLD}Starting Execution...${RESET}"
...
echo "${CYAN}${BOLD}Zone 1:${RESET} $ZONE_1"
echo "${CYAN}${BOLD}Region 1:${RESET} $REGION_1"
echo "${CYAN}${BOLD}Zone 2:${RESET} $ZONE_2"
echo "${CYAN}${BOLD}Region 2:${RESET} $REGION_2"

# Create the management network
echo "${YELLOW}${BOLD}Creating management network...${RESET}"
gcloud compute networks create managementnet --subnet-mode=custom

# Create the management subnet in the first region
echo "${YELLOW}${BOLD}Creating management subnet in $REGION_1...${RESET}"
gcloud compute networks subnets create managementsubnet-1 --network=managementnet --region=$REGION_1 --range=10.130.0.0/20

# Create the private network
echo "${YELLOW}${BOLD}Creating private network...${RESET}"
gcloud compute networks create privatenet --subnet-mode=custom

# Create the first private subnet in the first region
echo "${YELLOW}${BOLD}Creating private subnet 1 in $REGION_1...${RESET}"
...
echo "${YELLOW}${BOLD}Creating private subnet 2 in $REGION_2...${RESET}"
...
# Create Instances
echo "${YELLOW}${BOLD}Creating instances...${RESET}"
...
echo "${YELLOW}${BOLD}Creating appliance instance...${RESET}"
...
echo "${BG_GREEN}${WHITE}${BOLD}Subscribe! Cloudgoodies for more Solution!${RESET}"
#-----------------------------------------------------end----------------------------------------------------------#
