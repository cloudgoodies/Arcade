...
# ------------------------------------------------------
echo "${GREEN_TEXT}${BOLD_TEXT}👉 Fetching ZONE and REGION automatically...${RESET_FORMAT}"
# ------------------------------------------------------
...
# ------------------------------------------------------
gcloud compute networks create vpc-net --project=$DEVSHELL_PROJECT_ID --description="" --subnet-mode=custom


# ------------------------------------------------------
echo "${GREEN_TEXT}${BOLD_TEXT}👉 Creating VPC Subnet vpc-subnet with custom range...${RESET_FORMAT}"
# ------------------------------------------------------
...
echo "${GREEN_TEXT}${BOLD_TEXT}⏳ Waiting 100 seconds for network propagation...${RESET_FORMAT}"
sleep 100


# ------------------------------------------------------
echo "${GREEN_TEXT}${BOLD_TEXT}👉 Creating Firewall Rule allow-http-ssh...${RESET_FORMAT}"
# ------------------------------------------------------
...
# ------------------------------------------------------
echo "${GREEN_TEXT}${BOLD_TEXT}👉 Creating BigQuery dataset for VPC Flow Logs...${RESET_FORMAT}"
# ------------------------------------------------------
bq mk bq_vpcflows


# ------------------------------------------------------
echo "${GREEN_TEXT}${BOLD_TEXT}👉 Fetching Public IP of web-server...${RESET_FORMAT}"
# ------------------------------------------------------
...
# ------------------------------------------------------
echo "${GREEN_TEXT}${BOLD_TEXT}👉 Asking user to continue...${RESET_FORMAT}"
# ------------------------------------------------------
while true; do
...
echo
echo "${CYAN_TEXT}${BOLD_TEXT}   CONGRATULATION  LAB COMPLETED SUCCESSFULLY!! and for more video subscribe Cloudgoodies channel!!             ${RESET_FORMAT}"
echo
