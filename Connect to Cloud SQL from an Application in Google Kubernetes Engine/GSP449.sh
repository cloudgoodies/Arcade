...
echo
echo "${YELLOW_TEXT}${BOLD_TEXT}🛠️  Determining and setting the default Google Cloud Zone for this session...${RESET_FORMAT}"
...
echo
echo "${YELLOW_TEXT}${BOLD_TEXT}📁 Changing directory to the extracted demo application folder...${RESET_FORMAT}"
cd gke-cloud-sql-postgres-demo

echo
echo "${YELLOW_TEXT}${BOLD_TEXT}📧 Retrieving the active gcloud account email for PostgreSQL setup...${RESET_FORMAT}"
PG_EMAIL=$(gcloud config get-value account)
echo "${WHITE_TEXT}PostgreSQL admin email will be: ${BOLD_TEXT}$PG_EMAIL${RESET_FORMAT}"

echo
echo "${YELLOW_TEXT}${BOLD_TEXT}🚀 Executing the setup script for the demo application. This may take a few moments, please wait...${RESET_FORMAT}"
./create.sh dbadmin $PG_EMAIL

echo
echo "${YELLOW_TEXT}${BOLD_TEXT}⏳ Initializing resources...${RESET_FORMAT}"
for i in $(seq 10 -1 1); do
  echo -ne "${BLUE_TEXT}${BOLD_TEXT}⏳ ${i} seconds remaining...${RESET_FORMAT}\r"
  sleep 1
done
echo -ne "\n"
echo "${YELLOW_TEXT}${BOLD_TEXT}✅ Resources initialized.${RESET_FORMAT}"
...
echo
echo "${YELLOW_TEXT}${BOLD_TEXT}🌐 Exposing the application Pod (${POD_ID}) as a LoadBalancer service on port 80...${RESET_FORMAT}"
kubectl expose pod $POD_ID --port=80 --type=LoadBalancer

echo
echo "${YELLOW_TEXT}${BOLD_TEXT}📋 Displaying all services in the default namespace, including the new LoadBalancer...${RESET_FORMAT}"
kubectl get svc


echo "${GREEN_TEXT}${BOLD_TEXT}💖Congratulastion to Complete this lab!!👇${RESET_FORMAT}"
